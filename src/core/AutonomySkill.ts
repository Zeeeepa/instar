...
const PROFILE_DESCRIPTIONS: Record<AutonomyProfileLevel, {
  short: string;
  detail: string;
  riskNote: string;
}> = {
...
type AutonomyIntent =
  | { type: 'status' }
  | { type: 'set-profile'; profile: AutonomyProfileLevel }
  | { type: 'suggest-elevation' }
...
const INTENT_PATTERNS: Array<{
  patterns: RegExp[];
  intent: (match: RegExpMatchArray) => AutonomyIntent;
}> = [
  // Set autonomous
  {
    patterns: [
...
export class AutonomySkill {
  private deps: AutonomySkillDeps;
    ...
    if (dashboard.summary) {
      ...
      if (trustStart !== -1) {
        ...
        for (let i = trustStart + 1; i < summaryLines.length; i++) {
          ...
          if (line.startsWith('Elevation opportunities:') || line.startsWith('Pending evolution')) break;
   ...
   */
  private classifyIntent(message: string): AutonomyIntent {
   ...
   */
  private suggestElevation(): AutonomyResponse {
    const manager = this.deps.autonomyManager;
    const current = manager.getProfile();
    const order: AutonomyProfileLevel[] = ['cautious', 'supervised', 'collaborative', 'autonomous'];
   ...
   */
  private revertProfile(): AutonomyResponse {
    const manager = this.deps.autonomyManager;
   ...
   */
  private detectOverrides(resolved: ResolvedAutonomyState, _profile: AutonomyProfileLevel): string[] {
