...
export class ReflectionConsolidator {
  ...
  consolidate(opts?: ConsolidatorOptions): ConsolidationResult {
    ...
    const analyzerOpts: PatternAnalyzerOptions = { days, agentId, minRuns };
    ...
    const jobSummaries: JobConsolidationSummary[] = [];
    const allCreatedProposals: EvolutionProposal[] = [];
    let totalSkipped = 0;
    ...
    return {
      ...
      jobSummaries: jobSummaries.map(s => {
        ...
        const { _createdProposals, ...rest } = s as JobConsolidationSummary & { _createdProposals: EvolutionProposal[] };
        return rest;
      }),
  ...
  private processJobReport(
    ...
    commit: boolean,
  ): JobConsolidationSummary & { _createdProposals: EvolutionProposal[] } {
    const createdProposals: EvolutionProposal[] = [];
    let skipped = 0;
    ...
    for (const candidate of candidateProposals) {
      ...
      const isDuplicate = existingProposals.some(existing =>
  ...
  private titlesMatch(a: string, b: string): boolean {
    ...
    if (stepA && stepB && stepA === stepB) return true;
    return false;
  }
}
