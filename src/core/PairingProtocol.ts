 ...
 */
export function comparePairingCodes(a: string, b: string): boolean {
 ...
 */
const SAS_SYMBOLS: Array<{ word: string; emoji: string }> = [
  { word: 'wolf', emoji: '🐺' },
  { word: 'wave', emoji: '🌊' },
  { word: 'fire', emoji: '🔥' },
  { word: 'music', emoji: '🎵' },
...
export function deriveSAS(
  ...
  publicKeyA: string,
  publicKeyB: string,
): { symbols: Array<{ word: string; emoji: string }>; display: string } {
  ...
  const symbols: Array<{ word: string; emoji: string }> = [];
  for (let i = 0; i < 3; i++) {
    const byte = hash[i];
...
export function deriveSessionKey(
  ...
  const sessionKey = crypto.hkdfSync(
    'sha256',
    rawShared,
    Buffer.from(pairingCode.toUpperCase()),
    Buffer.from(info),
    32, // 256 bits
  );
 ...
 */
export function isPairingSessionValid(session: PairingSession): boolean {
  if (session.consumed) return false;
  if (session.failedAttempts >= session.maxAttempts) return false;
  if (Date.now() - session.createdAt > session.expiryMs) return false;
  return true;
}
