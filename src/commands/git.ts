...
import type { GitStateConfig } from '../core/types.js';
...
function loadGitManager(dir?: string): { manager: GitStateManager; config: ReturnType<typeof loadConfig> } {
  const config = loadConfig(dir);
  const gitConfig: Partial<GitStateConfig> = (config as any).git || {};
...
export async function gitInit(opts: GitOptions): Promise<void> {
  ...
  if (config.agentType === 'project-bound') {
    console.log(pc.red(
      'Git state tracking is only supported for standalone agents.\n' +
...
export async function gitStatus(opts: GitOptions): Promise<void> {
  ...
  if (status.remote) {
    ...
    if (status.ahead > 0) console.log(`  Ahead:     ${pc.green(`+${status.ahead}`)}`);
    if (status.behind > 0) console.log(`  Behind:    ${pc.yellow(`-${status.behind}`)}`);
  } else {
...
export async function gitPush(opts: GitOptions): Promise<void> {
  const { manager } = loadGitManager(opts.dir);
...
export async function gitPull(opts: GitOptions): Promise<void> {
  const { manager } = loadGitManager(opts.dir);
...
export async function gitLog(opts: GitOptions): Promise<void> {
  const { manager } = loadGitManager(opts.dir);
...
export async function gitRemote(url: string, opts: GitOptions): Promise<void> {
  const { manager } = loadGitManager(opts.dir);
...
export async function gitCommit(message: string, opts: GitOptions): Promise<void> {
  const { manager } = loadGitManager(opts.dir);
