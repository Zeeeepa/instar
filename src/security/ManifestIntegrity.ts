...
export class ManifestIntegrity {
   ...
   */
  private static sortedStringify(obj: unknown): string {
    ...
    const pairs = sortedKeys.map(
      key => JSON.stringify(key) + ':' + ManifestIntegrity.sortedStringify((obj as Record<string, unknown>)[key]),
    );
    return '{' + pairs.join(',') + '}';
  }
   ...
   */
  private computeHmac(manifest: Record<string, unknown>): string {
   ...
   */
  sign<T extends Record<string, unknown>>(manifest: T): T & { _hmac: string } {
  ...
  verify(manifest: Record<string, unknown>): boolean {
    ...
    return crypto.timingSafeEqual(
      ...
      Buffer.from(storedHmac as string, 'hex'),
    );
  }
   ...
   */
  readAndVerify(manifestPath: string): { manifest: SignedManifest | null; verified: boolean; error?: string } {
    ...
    try {
      ...
      const manifest = JSON.parse(content) as SignedManifest;

      const verified = this.verify(manifest as unknown as Record<string, unknown>);
