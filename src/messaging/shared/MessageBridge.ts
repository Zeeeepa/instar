...
import type { MessagingEventBus, MessageLoggedEvent } from './MessagingEventBus.js';

// ── Types ──────────────────────────────────────────────────

export interface BridgeLink {
  whatsappChannelId: string; // JID: "14155552671@s.whatsapp.net"
  telegramTopicId: number; // Telegram forum topic ID
  createdAt: string;
...
export class MessageBridge {
  ...
  private links: BridgeLink[] = [];
  private unsubscribers: Array<() => void> = [];
  private started = false;
  private messagesBridged = 0;
  private lastBridgedAt: string | null = null;
  ...
  start(): void {
    ...
    if (this.config.whatsappEventBus && this.config.sendToTelegram) {
      const unsub = this.config.whatsappEventBus.on('message:logged', (event) => {
        this.handleWhatsAppMessage(event).catch(err =>
          console.error(`[message-bridge] WhatsApp→Telegram error: ${err}`),
        );
      });
    ...
    if (this.config.telegramEventBus && this.config.sendToWhatsApp) {
      const unsub = this.config.telegramEventBus.on('message:logged', (event) => {
        this.handleTelegramMessage(event).catch(err =>
          console.error(`[message-bridge] Telegram→WhatsApp error: ${err}`),
        );
      });
      this.unsubscribers.push(unsub);
    }
  ...
  addLink(whatsappChannelId: string, telegramTopicId: number, createdBy: string): void {
    ...
    this.links = this.links.filter(
      l => l.whatsappChannelId !== whatsappChannelId && l.telegramTopicId !== telegramTopicId,
    );
  ...
  private async handleWhatsAppMessage(event: MessageLoggedEvent): Promise<void> {
    ...
    if (event.text.startsWith(BRIDGE_PREFIX_TG)) return;

    const topicId = this.getTelegramForWhatsApp(event.channelId);
    if (!topicId || !this.config.sendToTelegram) return;
  ...
  private async handleTelegramMessage(event: MessageLoggedEvent): Promise<void> {
    // Only bridge user messages
    if (!event.fromUser) return;

    // Loop detection
    if (event.text.startsWith(BRIDGE_PREFIX_WA)) return;
    ...
    if (!jid || !this.config.sendToWhatsApp) return;
